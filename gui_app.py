...
# 它的模块顶部完成，晚于 pyplot 的首次 import 就不生效了。
#
# 这批名字里有几个本文件已不再直接引用（FORM_LABEL_W / _CJK_FALLBACK 等）。
# 仍然照单 import，是因为 gui_app 现在是界面层的兼容入口：测试与外部脚本一直
# 按 ``gui_app.XXX`` 取值，逐个核对谁还有人用，不如整体保留。
from deltalab_ui.theme import (
    FORM_ENTRY_CHARS, FORM_HINT_GAP, FORM_INPUT_W, FORM_LABEL_GAP,
    FORM_LABEL_W, FORM_ROW_PADY, FORM_SECTION_GAP, FORM_SECTION_PAD,
    PALETTE,
    _AVAILABLE_FONTS, _CJK_CANDIDATES, _CJK_FALLBACK, _MONO_CANDIDATES,
    _MONO_FONT_FAMILY, _MPL_CJK_FP, _SYSTEM, _UI_FONT_FAMILY,
...
class BacktestApp(view_compare.ComparisonMixin, view_results.ResultsMixin,
                  tk.Tk):
