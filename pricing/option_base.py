...
class OptionBase:
    ...
    def get_greeks(self):
        ...
        # delta（中心差分）
        price_up = self._bumped_copy(s0=self.s0 + ds).get_price()
        price_dn = self._bumped_copy(s0=self.s0 - ds).get_price()
        delta = (price_up - price_dn) / (2 * ds)

        # gamma（二阶中心差分，步长 2*ds）
        price_up2 = self._bumped_copy(s0=self.s0 + 2 * ds).get_price()
        price_dn2 = self._bumped_copy(s0=self.s0 - 2 * ds).get_price()
        gamma = (price_up2 - 2 * price0 + price_dn2) / (4 * ds ** 2)

        # vega
        price_vup = self._bumped_copy(sigma=self.sigma + dz).get_price()
        price_vdn = self._bumped_copy(sigma=self.sigma - dz).get_price()
        vega = (price_vup - price_vdn) / (2 * dz)

        # theta
        price_theta = self._bumped_copy(**self._theta_overrides(dt)).get_price()
        theta = (price_theta - price0) / (dt / ANNUAL_DAYS)

        # rho
        price_rup = self._bumped_copy(r=self.r + dr).get_price()
        price_rdn = self._bumped_copy(r=self.r - dr).get_price()
