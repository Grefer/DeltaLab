...
| `N` | 杠杆倍数（跌破 K 后） | int | `2` |
| `cp` | 方向，下拉选择 `看涨 (Call)` / `看跌 (Put)`，内部映射为 `1` / `-1` | int | `1`（看涨） |
| `fix` | 区间赔付（`*_Fix` 子类型用）：标的落在 K~H 区间时每日结算的金额 | float | `0.0`。**0 是合法取值**（区间内那几天不结算） |
| `P` | 保障价格（`ASGQ_*P` 用）：熔断日起改用该价格结算 | float | `100.0`（= 默认入场价） |
| `amount` | 熔断赔付（`ASGQ_*F` 用）：熔断日起每日结算的金额 | float | `0.0`。**0 是合法取值**（熔断后不再有现金流） |
| `r` / `q` | 利率 / 分红 | float | `0.03` / `0.03` |
| `nPath` | MC 路径数 | int | `100000` |

这三项**都可以填 0**，且 0 有明确含义（区间赔付 0 = 区间内那几天不结算，熔断赔付 0 = 熔断后不再有现金流）。不需要它们的子类型填什么都不影响定价。
...
| 字段 | 含义 | 默认 |
|---|---|---|
| `文件` | CSV 路径，旁边是 `浏览…` 与 `模板…` 两个按钮 | 空 |
| `价格列` | 用作价格序列的列名；可编辑下拉，选完文件后候选就是该文件的表头 | `close` |
...
`模板…` 会写出一份可直接跑通的样例并把它选成当前文件，替换掉数据行即可：
...
样例给到 26 个交易日是有意的：默认香草期权期限是 22 交易日，模板短于这个长度
的话，「存模板 → 直接运行」第一下就会撞上「价格序列交易日组不足」——引导做到
一半反而把人卡在更难懂的报错上。自备文件同理，行情长度要够覆盖期权期限。

三条硬要求：

1. **第一列是日期索引**，日频写 `2024-01-02`，日内写完整时间戳
   `2024-01-02 09:31:00`；每日 bar 数由时间戳自动推导，不必也无法手填。
2. **其余列任取**，`价格列` 指到哪列就用哪列，`close` 只是默认名；表头是中文
   （如 `收盘价`）同样可用。
3. **至少 2 行、按时间正序**；不足 2 行 `from_csv` 直接拒绝。

模板不带注释行——`pd.read_csv` 默认不跳 `#`，加了注释这份模板自己就读不进来，
所以格式说明只在界面提示与本节里。

选完文件后 `_browse_csv` 会读一次表头（`utf-8-sig` → `gbk`），把可用列灌进
`价格列` 下拉；当前列名不在表头里就自动切到 `close`、没有 `close` 则取第一
列，并在状态栏说明切换原因。改这条之前，列名填错要等点了 `运行` 才从
`from_csv` 收到「列 X 不在 CSV 中」，反馈隔了一整趟回测。
...
| `合约乘数` | `multiplier`，一手对应多少份标的，用于把目标持仓向零取整到整手；`0` = 不取整（连续对冲）。它只决定**取整粒度**，不放大交易规模——规模由 `交易数量` 单独决定 | `0` 或有限正数 | `5` |
...
**单位**：增量收益是**金额**，随 `数量` 线性放大，与「增量成本」列同单位；
增量信噪比是**无量纲比值**，与仓位大小无关。两者都不是百分比。

> 金额**不随合约乘数放大**。实际持仓是 `round(方向 × Δ × 数量 / 乘数) × 乘数`，
> 乘数在里面约掉了，它只把持仓对齐到整手；量级完全由 `数量` 决定。改乘数只会
> 让金额因取整落点不同而小幅抖动，不会成比例变化。
...
取整粒度固定，因此段内 PnL 量级正比于该段 `S0`——价格高的段权重更大。这不是失真：
...
路径和多代理段中位路径都使用各段
`PnL / (S0 × |quantity|)` 安全归一化（schema `s0_x_abs_quantity_v2`）。分母是
这次交易的名义敞口，**不含合约乘数**——乘数只决定取整粒度，把它乘进分母会让同一
份持仓、同一份盈亏在不同取整粒度下得出成比例不同的归一化结果（实测同一笔
`quantity=10000`、持仓 5088.5、PnL≈4340 的交易，乘数 0.01/0.02/0.05 下 v1 的归一化
值差 5 倍）。若 `S0` 或数量不能形成有限正分母，或同代理段候选与 C2C 的分母不一致，
界面会停用相关聚合图并提示改用单代理段路径，不会回退到跨合约原始金额曲线。

> **口径变更（v2）**：此前的分母是 `S0 × multiplier × |quantity|`
> （`s0_x_multiplier_x_abs_quantity_v1`）。按默认乘数 5，新口径的归一化数值是
> 旧口径的 5 倍。快照里记着各自的 schema，跨版本对比归一化列时要先看 schema
> 是否一致；金额列不受影响。
...
GUI 与底层 `HedgeBacktest.__init__` 均默认 `合约乘数 = 5`。新手第一次跑可能看到「Delta 与实际持仓」图上有明显阶梯——这是按 5 取整造成的，并非 bug。如需连续对冲请改为 `0`。
