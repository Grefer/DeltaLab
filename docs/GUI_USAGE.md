...
| `头寸方向` | 所有 GUI 回测唯一的方向来源；`卖出 (short)` / `买入 (long)` 内部分别映射为 `1` / `-1` | Radiobutton | `卖出 (short)` → `1` |
...
- **固定基准**：买卖方向分组；当前方向最早保留的 `close_to_close` 快照固定为
  基准。选择另一方向结果时会切换整组，并自动带入该方向基准；买入与卖出结果
  不能进入同一排名。
- **删除**：从当前会话移除快照；删除固定基准后，下一条已保留的
  `close_to_close` 自动接替。若没有可接替结果，绝对排名与曲线仍保留，但相对指标
  显示 `—` 并提示先保留每日收盘结果。
- **全选 / 清空**：“全选”只选择当前聚焦结果对应的同方向快照，相反方向不会
  混入；“清空”可以连同固定基准一起清空显示。之后再选择任一候选会自动带回
  同方向基准。选择 0 条时显示操作引导；只有基准 1 条时展示自身指标并提示再选一条。
- **产生方式**：`产生方式` 列显示这条快照是怎么来的 —— `手工回测`、
  `优选验证·<周期>` 或 `分段重放`。它读的是快照上的结构化 `origin` 字段，
  不是名称前缀，因此重命名不会让来源丢失。优选验证的快照还在 `origin_meta`
  里记录来源周期、当时排名、改善幅度和批次编号；这些只作溯源，不参与本页
  任何排名计算，本页指标始终由快照自身的单路径结果算出。
...
系统先强制要求头寸方向一致，再核对三组上下文：实际价格与时间索引、期权结构
参数、数量 / 乘数 / 成本 / 滑点 / 日级聚合口径。除方向外的上下文全部一致时
显示绿色确认；任一组不同时仍允许把曲线并列展示，但会说明排名只适合作为场景
对照，不能把全部差异归因于对冲策略。
...
| 较每日收盘改善↑ | `(基准 RMS - 本行 RMS) / |基准 RMS|`；**正值**表示 RMS 比 close-to-close 更低即更好，基准行显示“基准” |
...
`较每日收盘改善↑` 与 `策略优选` 页的同名指标方向一致（正值更好），顶部“当前
最优”卡片用的也是这一个定义。但两者不是同一个统计量：本页分母是 `|基准 RMS|`、
样本是一条单路径；优选页是以 `max(基准, 候选)` 为分母的有界改善、样本是严格
区间内的共同配对评分日。详情区列出的是原始差值（`本行 - 基准`），因此对 RMS、
...
| Wind 历史范围 | 独立指定分析截至日；自动模式取得 `max(L_selected) + V` 个评分/预热交易日，并额外请求一个 Day 0 收盘锚点。代理期限 `T` 不扩张证据区间；默认全选时 `max(L_selected)=243`，未使用 realized σ 时 `V=0` |
...
| 评价口径 | 候选与固定 C2C 基准只比较双方成功的共同代理段与共同评分日；主指标统一显示为“较 C2C 改善”。单一标的序列使用完整 `L` 日合并 RMS 的有界改善；品种历史合约池使用各非重叠代理段有界改善按实际评分日数加权，并区分正式历史参考、区间诊断与部分配对 |

核对基准环境、分析周期和候选配置后，点击 `开始历史择优` 才会读取真实历史并启动严格区间搜索；
...
realized 模式使用 Day 0 之前的独立预热收益种子；预热行情不进入严格区间 PnL，
...
双方都成功的共同代理段和共同评分日。单一标的序列的周期主指标，是完整严格
`L` 日合并日净 PnL RMS 的有界改善：

`I_L = (C2C RMS_L - 候选 RMS_L) / max(C2C RMS_L, 候选 RMS_L)`

双方 RMS 都为 0 时记为 0。正值表示候选在最近 `L` 日严格区间内的日净 PnL
RMS 更低。

品种历史合约池不能直接用跨合约的合并金额 RMS 排名，因为不同合约的价格与金额
尺度可能不同。系统先在每个非重叠代理段 `j` 内用同一公式计算有界改善 `I_j`，
再按该段实际评分日数 `d_j` 加权：

`I_L(pool) = Σ(d_j × I_j) / Σd_j`

因此品种池结果表中的策略 / C2C 汇总金额 RMS 只用于诊断，不参与主排名；主排名
看“较 C2C 改善”。代理段胜率也只作辅助诊断，不再声称“多个重叠终点的逐窗等权
优势”是主排名依据。若改善与 C2C 持平，
固定优先 C2C，避免为无历史改善的候选增加复杂度。

候选必须覆盖该周期全部 C2C 基准代理段和评分日，才可以进入正式历史择优；历史区间
本身不足但候选与现有 C2C 代理段完全配对时，只给“区间诊断”；候选自身缺 Bar 或
失败造成的部分配对只能展示诊断明细，不能成为周期领先者。

周期结论直接展示历史最优策略、策略/C2C 汇总 RMS、较 C2C 改善、代理段胜率
和配对代理段数；详情同时披露 `T / L / 实际评分日 / 代理段数 / 到期段 / MTM 段`
及终止口径。品种池还披露实际参与该周期评分的具体合约，并将汇总金额 RMS 明确
标为诊断值。只有至少一个非
...
每个已选周期先展示完整策略排名数据表，图表放在数据表下方，并始终以共同代理段的每日
收盘 close-to-close 结果为固定基准。排名行保持单选，用于查看详情、应用策略或执行
当前路径验证；勾选与其分离，通过排名表首列 `勾选` 选择候选。该勾选同时决定图表
叠加哪些曲线，以及“当前路径验证”按批量还是单条执行。切换周期时默认
叠加排名靠前且存在共同代理段的两个候选，可用 `勾选前2名` 恢复默认，`清空勾选`
清除全部，也可逐项调整；
C2C 不可取消，候选最多同时显示三个，以避免曲线和图例失去辨识度。候选的颜色和
...
历史排名不会直接写入回测结果池。排名表下方的动作栏把历史证据回放与“换到当前
环境重跑”收拢在同一行：

- **加载分段到展示页**：从左侧“历史分段”下拉选择该候选跑过的某一段历史，按原始
  行情切片重跑并把逐 bar 结果渲染到回测摘要 / 对冲图表 / 每日明细 / 波动率分析。
- **仅回填参数**：把策略类型及其专属参数回填到单次回测区，不立即运行，
...
- **当前路径验证**：使用当前单次回测环境执行推荐策略；只有回测成功后，才把它
  ...
  带上结构化来源（`优选验证·<周期>`）以及来源周期、当时排名与批次编号。

批量验证结束后不会自动跳页，也不再用模态框报错 —— 当前选定的周期和勾选状态
保持原样。结论写在动作栏下方的结果条里：全部成功显示绿色的完成条，有失败时
...
严格区间 Day 0 前具备完整 `V=N` 日预热；不足时涉及的代理段明确失败，不会回退 implied，也
