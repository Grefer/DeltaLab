...
两个动作按钮作用的对象就是选中行，两者必须指同一个策略。跨周期一致性收敛为周期
切换条右端的一枚 pill（一致 / 不一致），完整措辞在悬停提示里。

历史排名不会直接写入回测结果池。“换到当前环境重跑”的两个按钮在结论卡右侧，历史
证据回放因为要先在图上选段，仍留在图表下方：

- **加载分段到展示页**（图表下方）：从“历史分段”下拉选择该候选跑过的某一段历史，
  按原始行情切片重跑并把逐 bar 结果渲染到回测摘要 / 对冲图表 / 每日明细 / 波动率分析。
