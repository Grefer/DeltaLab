...
那一页排名看的是增量收益 / 增量性价比，两者既非同一统计量也非同一样本：本页
...
| 评价口径 | 候选与固定 C2C 基准只比较双方成功的共同代理段与共同评分日。单一标的序列按**增量收益 / 增量性价比**排名（各段日净 PnL 接续后逐日相减）；品种历史合约池跨合约无法相加金额，改用各非重叠代理段有界改善按实际评分日数加权。三种状态区分正式历史参考、区间诊断与部分配对 |
...
- `增量性价比 = mean(逐日增量) / std(逐日增量)`

两个口径都在回测时一并算出，结果页点表头即可切换，不必重跑。增量性价比恰好
等于配对 t 值除以 `sqrt(n)`，因此它在给出性价比的同时也编码了这份增量有多
可靠——样本内偶然赚到的一两天大钱会被自身波动稀释。
...
因此品种池结果表里“增量收益 / 增量性价比 / 多花成本”三列都显示 `—`，两列表头
...
排名表展示的四列是增量收益、增量性价比、多花成本、最大回撤；RMS 得分与代理段
