...
本文档面向首次使用 `gui_app.py` 的同事，目标是：能够从零启动 GUI、跑通一次完整回测、并理解界面上每一个参数的含义、单位、默认值与对结果的影响。
...
| 库 | 触发条件 | 不装时的影响 |
|---|---|---|
| `WindPy` | 在「数据来源」选 `Wind` | 选 Wind 模式运行时会抛 `ImportError`（提示见 `pricing/wind_data.py:28`），需安装 Wind 终端 + Python 插件，并已登录 |

CSV 与模拟模式无 Wind 依赖，可在普通环境下完整使用。

### 1.4 字体

GUI 在启动时会按平台自动挑选可用 CJK 字体（`gui_app.py:24-46`）：
...
入口为 `gui_app.py:1932` 的 `main()`。窗口默认大小 `1600x1000`，最小尺寸 `1200x720`（`gui_app.py:317-323`）。

### Windows 注意事项

- 路径中含中文一般没问题，但若遇到 Wind/CSV 加载失败，可优先排查路径。
- 高 DPI 屏幕下，左侧参数面板已包了 Canvas + Scrollbar（`gui_app.py:580-627`），可滚轮浏览，不会出现底部按钮被裁的情况。

---

## 3. 界面总览

主窗口分为左右两栏（`ttk.PanedWindow`，`gui_app.py:572`），可拖动中间分隔条。`_build_ui` 方法从 `gui_app.py:556` 开始。

### 3.1 左侧：参数与运行区

由上至下分四块：

1. **期权类型** —— 选大类与子类型（`gui_app.py:633`）
2. **期权参数** —— 随期权大类动态生成的输入框（`gui_app.py:656`）
3. **回测设置** —— 数据来源、对冲策略、滑点、合约乘数等（`gui_app.py:663`）
4. **运行按钮区** —— `运行回测` / `绘制结构图` / 进度条（`gui_app.py:847`）

### 3.2 右侧：结果标签页（`ttk.Notebook`，`gui_app.py:879`）
...
底部为状态栏（`gui_app.py:927`），实时显示「就绪 / 正在运行 / 完成」。
...
| 控件 | 选项 | 默认 | 说明 |
|---|---|---|---|
| `大类` | `香草期权 (Vanilla)` / `累计期权 (Decumulator)` / `亚式期权 (Asian)` / `气囊期权 (Airbag)` | `香草期权 (Vanilla)` | 见 `OPTION_CLASSES`，`gui_app.py:108` |
| `子类型` | 跟随大类切换 | 各大类的第一个 | 子类型驱动 `optiontype` 字段，决定走哪一种 payoff |

各大类的子类型集（`gui_app.py:109-207`）：
...
每个子类型的 payoff 公式见 `STRUCTURE_DOCS`（`gui_app.py:214-308`），也会在「结构分析」Tab 顶部展示。

### 4.2 期权参数（按大类分组）

每个字段都是一个 Entry，初始值取自 `OPTION_CLASSES[...]["params"]` 的 default。空字符串会被解释为 0（见 `_collect_gui_state`，`gui_app.py:1010`）。
...
定价走 BS 封闭解（`Option_Vanilla.blsprice`，`Option_Vanilla.py:69`）。
...
注意 `fix / P / amount` 的「0 当 None」是在 `OPTION_CLASSES["累计期权"]["build"]` 里做的（`gui_app.py:155-157`），对应不需要这些字段的子类型可以保留 0。
...
观察日默认取 `range(1, T_days+1)`（每日观察），见 `OPTION_CLASSES["气囊期权"]["build"]`（`gui_app.py:200`）。

### 4.3 回测设置 — 数据来源

`数据来源` 是单选 Radiobutton（`gui_app.py:670-675`），三选一：
...
切换数据来源时，`_toggle_source`（`gui_app.py:960`）会动态显示对应参数子区，并对 csv / wind 强制锁定 `steps_per_day=1`（实盘/CSV 仅支持日频）。

#### 模拟（simulate）

| 字段 | 含义 | 默认 |
|---|---|---|
| `种子` | `numpy` `default_rng` 种子 | `42` |
...
底层走 `HedgeBacktest.from_csv`（`hedge_backtest.py:1028`）。日期列默认取第一列（`pd.read_csv(parse_dates=[0])`）。
...
底层走 `HedgeBacktest.from_wind`（`hedge_backtest.py:946`），复权方式硬编码为 `"F"`（前复权）。

> 真实行情模式下，期权参数中的 `s0` 视为「参考价 S_ref」；GUI/底层会按 `ratio = 真实首日价 / S_ref` 自动缩放 `K / KI / H / P / fix / amount` 等价格量纲字段，保证期权结构不被破坏。详细缩放表会展示在「回测摘要」Tab 中（`gui_app.py:1271-1284`，逻辑在 `hedge_backtest.py:_rescale_option_to_real_s0`，`hedge_backtest.py:181`）。
...
切到 `sigma_band` 时，下方多出 3 个字段（`gui_app.py:799-816`），切换由 `_toggle_strategy`（`gui_app.py:981`）控制：

| 字段 | 含义 | 取值 | 默认 |
|---|---|---|---|
| `σ 带 k` | 触发阈值倍数 | float > 0 | `0.5` |
| `σ 来源` | 用什么 σ 估计触发阈值 | `implied` / `realized` | `implied` |
| `HV 窗口 N 日` | `realized` 时滚动窗口长度（单位=日） | int ≥ 2 | `20` |

触发条件（来自 `hedge_backtest.py:SigmaBandStrategy.should_hedge`，`hedge_backtest.py:140-155`）：

```
|ln(S / S_last)| >= k * σ_ref * sqrt(dt_since_last)
```

其中 `σ_ref` 取 `option.sigma`（implied）或最近 `window_days` 个交易日对数收益的年化 std；`realized` 模式下样本不足时回退到 implied（`hedge_backtest.py:129-138`）。

`fixed_freq` 模式下，`SigmaBandStrategy` 控件被隐藏，调仓频率取自 `调仓频率(天)`（每 N 个 bar 调一次仓，`hedge_backtest.py:FixedFreqStrategy`，类定义于 `hedge_backtest.py:48`）。
...
下拉提示文字：`1=日频 / 4=60分 / 48=5分 / 240=1分`（`gui_app.py:827`）。
...
> CSV / Wind 模式下该控件被禁用并强制为 `1`（实盘/CSV 仅支持日频，`gui_app.py:967-975`、`hedge_backtest.py:1007`（`from_wind`）/ `hedge_backtest.py:1068`（`from_csv`））。

跨日 bar 上 `option.step_forward` 被调用；日内 bar 仅用 `_bumped_copy(_intraday_elapsed=…)` 临时评估 Δ，不污染 option 内部状态（`hedge_backtest.py:500-516`）。年化口径 `dt_bar = 1 / (ANNUAL_DAYS * spd)`，已实现波动率年化因子 `ANNUAL_DAYS * spd`。
...
> 结构图默认会限制 `nPath ≤ 20000` 以加速扫描（`gui_app.py:1732`），原值若大于 20000 仅在结构图时生效，回测主流程仍用原值。

> 结构图视角与 `头寸方向` 联动：卖方（`position=1`）时 `sign=-1`，所有曲线乘以 `sign`（`gui_app.py:1795-1802`），方便直观判断卖方账户的盈亏方向。
...
### 6.1 回测摘要（`gui_app.py:1227`，`_show_summary`）

- **单路径盈亏分解**：
  - `标的对冲盈亏` = `Σ H[i-1]·(S[i]-S[i-1])` —— 标的腿浮动盈亏
  - `期权 MtM 盈亏` = `Σ -position·(V[i]-V[i-1])·quantity` —— 期权方价值变动（卖方视角下取负）
  - `累计交易成本` = `Σ TC[i]`
  - `对冲误差` = `position·V[0]·quantity + Σ hedge_daily − Σ TC − position·V[-1]·quantity`，对应 `hedge_backtest.py:602`
- **波动率分析**：`成交隐含波动率` 直接来自 `option.sigma`；`已实现波动率` 用全样本 bar 级对数收益年化。
- **Greeks 统计**：表格列出 Delta/Gamma/Vega/Theta/Rho 的初始值、序列均值、最大绝对值。

### 6.2 对冲图表（`gui_app.py:1355`，`_show_chart`）

6 宫格，从左上到右下：标的价格、Delta 持仓目标 vs 实际持仓、Gamma、Vega、Theta、累计盈亏（绿/红填色表示正负）。

> Delta 持仓目标 = `delta × quantity × position`（`gui_app.py:1387`），实际持仓 `shares` 受 `multiplier` 取整影响，会出现明显阶梯。

### 6.3 波动率分析（`gui_app.py:1446`，`_show_vol_chart`）

- (1) 滚动 RV(20d) vs IV，填色表示 RV 与 IV 大小关系（红=RV>IV）。
- (2) 累计 RV 与 IV 的收敛对比。
- (3) 价差时间序列 = IV − 滚动 RV，正值（绿色）表示卖方相对有利。
- (4) 日收益直方图叠 IV 正态、RV 正态；其中日 IV σ = `IV / sqrt(ANNUAL_DAYS)` 转换为百分比（`gui_app.py:1524`，`ANNUAL_DAYS=243`，见 `pricing/constants.py`）。

### 6.4 盈亏分布（`gui_app.py:1544`，`_show_dist_chart`）
...
### 6.5 结构分析（`gui_app.py:1782`，`_show_structure`）

顶部有 `STRUCTURE_DOCS` 中的中文 payoff 描述 + 当前参数摘要；下方 6 宫格扫描曲线，含 `K/H/KI/E/P` 等关键价位的虚线标记（颜色区分）。视角受头寸方向控制（`gui_app.py:1795-1802`）：卖方（`position=1`）时 `sign=-1`，买方（`position=-1`）时 `sign=1`，所有曲线均乘以 `sign`，方便直观判断。

### 6.6 每日明细（`gui_app.py:1635`，`_show_table`）

Treeview 形式的每日表格，含 13 列：标的价格 / 期权价值 / Δ/Γ/ν/Θ/ρ / 持仓 / 标的盈亏 / 期权盈亏 / 交易成本 / 每日净盈亏 / 累计盈亏。可点 `导出 CSV` 用 `utf-8-sig` 编码保存（Excel 友好，`gui_app.py:1919`）。
...
- **`ImportError: WindPy`**：仅在选 Wind 数据源时触发（`pricing/wind_data.py:28`）。模拟和 CSV 模式不需要 Wind。
...
来源：`hedge_backtest.py:405`。CSV / Wind 模式下需要保证起止日范围内的交易日数 ≥ 期权剩余期限。可以放宽日期区间或调小 `T_days / T`。

### 7.3 历史价 S_ref 必须 > 0

CSV / Wind 模式下若期权 `s0` 设为 0 或负数，会触发 `_rescale_option_to_real_s0` 抛错（`hedge_backtest.py:200-204`）。请填入正的参考价。

### 7.4 `每日 bar 数` 在 CSV / Wind 模式下被锁死

Wind / CSV 真实行情仅支持日频；`_toggle_source` 会强制 `spd_var=1` 并禁用下拉框，UI 提示文字变为「实盘/CSV 模式仅支持日频 (spd=1)」。即便用户绕过 GUI 直接调用 `from_wind(steps_per_day=4)`，底层也会强制改为 1 并打印日志（`hedge_backtest.py:1007`）。
...
`_structure_worker`（`gui_app.py:1724`）会把 `nPath > 20000` 强制截断为 20000（`gui_app.py:1732`）以加速扫描。这只影响结构图，不影响主回测中真实的 `nPath`。

### 7.7 `HV 窗口 N 日` 样本不足

`SigmaBandStrategy(sigma_source='realized')` 在窗口内有效样本 < 2 时（例如刚开始几个 bar）会自动回退到 implied σ（`hedge_backtest.py:129-138`），不会抛错。

### 7.8 `multiplier` 默认值 = 5

GUI 与底层 `HedgeBacktest.__init__` 均默认 `合约乘数 = 5`（`gui_app.py:775`、`hedge_backtest.py:329`）。新手第一次跑可能看到「Delta 与实际持仓」图上有明显阶梯——这是按 5 取整造成的，并非 bug。如需连续对冲请改为 `0`。
