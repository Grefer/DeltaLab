...
class ComparisonMixin:
    ...
    # 「期权类型」的上限按最长的子类中文名留：「敲出计零·区间固赔·到期杠杆
    # 累计」在单元格字体（Menlo 9）下实测 131px，加 16px 内边距是 147——150
    # 正好装得下。结构改名把最长名从 9 个汉字拉到 14 个汉字加 2 个间隔点，这
    # 一列却不用动：等宽中文在 Menlo 9 下每字约 9px，比历史结果窗口那张表用
    # 的 TkDefaultFont 窄得多（同一串在那边要 189px，所以那边得给 210）。
