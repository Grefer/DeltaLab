...
- **回测结果池对比**：逐次回测后按需保留结果（自动存本机，重开仍在），勾选任意组合即时比较绝对指标，并说明这次对比的变量差在哪一项；右键可查看任一条的全部原始参数
...
6. 打开 **🆚 结果对比**，勾选快照即时查看累计净 PnL、指标表和这次对比的变量；右键某条可看它的全部原始参数
...
两者口径不同，不会混排。结果对比页不替你下判断：任意组合都能勾选，页面负责**说清这次的变量差在哪一项**（恰好一项不同、两项以上不同、完全相同三种结论），并提示会误导读数的情况——交易日数不同、或同时勾了买入与卖出（那时期末净损益与最大回撤的正负来自方向本身，横向应比总成本、再触发与换手额）。
...
- **衔接动作** — `加载明细`（图表下方选段，把**已落盘的**逐 bar 明细渲染到摘要 / 图表 / 每日明细并自动跳转，可逐条查看建仓、触发与终止记录；正常是读盘约 3 ms，只有明细确实不在时才按配方重算并补存）、`应用策略到左侧参数`（只写回对冲策略，不动期权 / 方向 / 行情，也不代跑）、排名行右键 `参数详情`（列出这一行背后的全部输入：公共的期权与行情、这条候选自己的策略参数、本行的取样口径）。想让某个候选进入结果对比页，走普通路径——应用参数 → `▶ 运行回测` → `＋ 保留当前结果到对比`。
