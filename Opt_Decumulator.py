...
def Opt_ASGQ_call_put(
    ...
    if T_Days == 0:
        ...
        if cp == 1:
            idx_ko = np.where(sr >= H)
            condition_ko[idx_ko] = 1
            flag_N = sr[-1] <= K if len(idx_ko) == 0 else False
            cashflow = ((sr[-1] - P) * condition_ko + (sr[-1] - K) * ~condition_ko) * (flag_N * N + ~flag_N * 1)
        else:
            idx_ko = np.where(sr <= H)
            condition_ko[idx_ko] = 1
            flag_N = sr[-1] >= K if len(idx_ko) == 0 else False
            cashflow = ((P - sr[-1]) * condition_ko + (K - sr[-1]) * ~condition_ko) * (flag_N * N + ~flag_N * 1)

        price = np.sum(cashflow)

    else:
        condition_ko = np.zeros([nPath, le], dtype=bool)
        flag_N = np.zeros([nPath, 1], dtype=bool)
        discount_factor = np.tile(np.exp(-r * (np.maximum(Observ - T_over, 0)) * dt), (nPath, 1))
        S = McGbmQ(s0, r - q, sigma, T, nPath, nStep)
        ss = np.c_[np.tile(sr, (nPath, 1)), S]
        if cp == 1:
            ...
            flag_N[(ss[:, -1] <= K) & (np.sum(condition_ko, axis=1) == 0)] = 1
            cashflow = ((ss[:, -1] - P).reshape(nPath,1) * condition_ko + (ss[:, -1] - K).reshape(nPath,1) * ~condition_ko) * (flag_N * N + ~flag_N * 1) * discount_factor
        else:
            condition_ko[np.cumsum(ss <= H, axis=1) > 0] = 1
            flag_N[(ss[:, -1] >= K) & (np.sum(condition_ko, axis=1) == 0)] = 1
            cashflow = ((P - ss[:, -1]).reshape(nPath,1) * condition_ko + (K - ss[:, -1]).reshape(nPath,1) * ~condition_ko) * (flag_N * N + ~flag_N * 1) * discount_factor
...
if __name__ == "__main__":
    ...
    price = Opt_ASGQ_call_put(100, [], 90, 100, 0, 20, list(range(1, 21)), 0.03, 0.03, 0.18, 110, 2, 100000, 1)
